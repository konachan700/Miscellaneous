...
void mew_i2c1_init() {
	...
	i2c_set_own_7bit_slave_address(I2C1, 0xc8);
...
enum MEW_I2C_RETVAL mew_i2c_write(uint32_t i2c_int, uint8_t da, uint8_t *buf, uint32_t buf_len) {
    ...
    while ((I2C_SR1(i2c_int) & 0x0001) != 0x0001) {
        if (Timeout-- == 0) return GENERAL_ERROR_WRITE_1;
    }

    //da &= 0xFFFE;
    //device_address = da;
    I2C_DR(i2c_int) = 0xc8; // da; // & 0xFFFE;

    Timeout = 0xFFFF;
    while ((I2C_SR1(i2c_int) & 0x0002) != 0x0002) {
        if (Timeout-- == 0) return GENERAL_ERROR_WRITE_2;
    }
    ...
    while (buf_len--) {
    	Timeout = 0xFFFF;
        while ((I2C_SR1(i2c_int) & 0x0004) != 0x0004) {
        	if (Timeout-- == 0) return GENERAL_ERROR_WRITE_2;
        }
        I2C_DR(i2c_int) = *buf;
        buf++;
...
enum MEW_I2C_RETVAL mew_i2c_read(uint32_t i2c_int, uint8_t da, uint8_t *buf, uint32_t buf_len) {
    ...
    if (buf_len == 1) {
        ...
        I2C_CR1(i2c_int) |= 0x0200;
        __enable_irq();
        while ((I2C_SR1(i2c_int) & 0x0040) != 0x0040);
        *buf = I2C_DR(i2c_int);
