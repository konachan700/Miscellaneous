...
//    public final ImageIcon ICON_TRAF            = new javax.swing.ImageIcon(getClass().getResource("/icons_16x16/office-chart-area-stacked.png"));
    ...
    public static final int 
            SLEA_DELETE_BUTTON      = 9999,
            GYNBE_YES_BUTTON        = 9998,
            GYNBE_NO_BUTTON         = 9997,
            SLEA_OPTIONS_BUTTON     = 9996,
            COMBO_BOX_BTN1          = 9995
            ;
    ...
    private void _checkIDGID(String ID, String GID) {
        if (ListHeaders.get(GID) == null) throw new NullPointerException("Group "+GID+" not exist in xlist!");
        if (this.GetItem(GID, ID) != null) throw new RuntimeException("GID:"+GID+" ID:"+ID+" element already exist in xlist!");
    }
    ...
    public void AddItem(String ID, String GID, Component element, String tags, int SortIndex) {
        _checkIDGID(ID, GID);
        final XListElementContainer el = new XListElementContainer();
    ...
    public void AddItemForSelector(String ID, String GID, String title, String tags, ImageIcon icon) {
        _checkIDGID(ID, GID);
        final GlobalMainMenuElement elx = new GlobalMainMenuElement(GID, ID, title, icon);
    ...
    public void AddItemForSimpleList(String ID, String GID, String title, ImageIcon icon, String tags) {
        _checkIDGID(ID, GID);
        final GlobalMainMenuElement elx = new GlobalMainMenuElement(GID, ID, title);
        elx.SetSelectedBackgroundColor(ListHeaders.get(GID).getBackground().darker()); 
        elx.SetIcon(icon);
        this.AddItem(ID, GID, elx, tags, 0);
    }
    ...
    public void AddSettingsElementText(String ID, String GID, String title) {
        _checkIDGID(ID, GID);
        final SettingsTextBoxElement sit = new SettingsTextBoxElement(GID+"_"+ID, title);
        this.AddItem(ID, GID, sit, title, 0);
    }
    
    public void AddSettingsElementTextNS(String ID, String GID, String value, String title) {
        _checkIDGID(ID, GID);
        final SettingsTextBoxElement sit = new SettingsTextBoxElement(title, value, false);
        this.AddItem(ID, GID, sit, title, 0);
    }
    
    // TODO: public void AddSettingsElementNumber()
    
    public void AddSettingsElementNumberNS(String ID, String GID, String title, long value, long _min, long _max) {
        _checkIDGID(ID, GID);
        final SettingsNumberBoxElement sit = new SettingsNumberBoxElement(title, value, _min, _max);
        this.AddItem(ID, GID, sit, title, 0);
    }
    
    public void AddSettingsElement2Buttons(String ID, String GID, String yesBtnTitle, String noBtnTitle, XListButtonsActionListener listener) {
        _checkIDGID(ID, GID);
        final GlobalYesNoButtonsElement yne = new GlobalYesNoButtonsElement(yesBtnTitle, noBtnTitle, listener);
        this.AddItem(ID, GID, yne, "", 0);
    }
    
    // TODO: AddSettingsElementComboBox()
    
    public void AddSettingsElementComboBoxNS(String ID, String GID, String title, Map<String,String> data, int selectedIndex) {
        _checkIDGID(ID, GID);
        final SettingsComboBoxElement cbe = new SettingsComboBoxElement(title, data, selectedIndex);
        this.AddItem(ID, GID, cbe, title, 0);
    }
    
    public void AddSettingsElementComboBoxNS(String ID, String GID, String title, Map<String,String> data, int selectedIndex, XListButtonsActionListener listener) {
        _checkIDGID(ID, GID);
        final SettingsComboBoxElement cbe = new SettingsComboBoxElement(title, data, selectedIndex, listener);
        this.AddItem(ID, GID, cbe, title, 0);
    }
    
    public void AddSimpleElementA(String ID, String GID, String title, String mark, XListButtonsActionListener listener) {
        _checkIDGID(ID, GID);
        final SimpleListElementA slea = new SimpleListElementA(ID, GID, title, mark, listener);
        this.AddItem(ID, GID, slea, title, 0);
    }
    
    public void AddSimpleElementA(String ID, String GID, String title, String mark, XListButtonsActionListener listener, boolean OptVisible, boolean DelVisible) {
        _checkIDGID(ID, GID);
        final SimpleListElementA slea = new SimpleListElementA(ID, GID, title, mark, listener, OptVisible, DelVisible);
        this.AddItem(ID, GID, slea, title, 0);
    }
    
    public void AddSettingsI2CAddressBoxElement(String ID, String GID, String title) {
        _checkIDGID(ID, GID);
        final SettingsI2CAddressBoxElement slea = new SettingsI2CAddressBoxElement(title);
        this.AddItem(ID, GID, slea, title, 0);
    }
    
    public void AddListElementI2CRegister(String ID, String GID, String addr, String min, String max, String inital, String type, String mark, XListButtonsActionListener li) {
        _checkIDGID(ID, GID);
        final ListElementI2CRegister slea = new ListElementI2CRegister(ID, GID, addr, min, max, inital, type, mark, li);
        this.AddItem(ID, GID, slea, "", 0);
    }
    
    public void AddSettingsHexNumberBoxElementNS(String ID, String GID, String title, int value, int _min, int _max) {
        _checkIDGID(ID, GID);
        final SettingsHexNumberBoxElement sit = new SettingsHexNumberBoxElement(title, value, _min, _max);
        this.AddItem(ID, GID, sit, title, 0);
    }
    
    public void AddSettingsSliderElementA(String ID, String GID, String title, int addr, int value, int _min, int _max, String type, XListSliderElementActionListener _li) {
        _checkIDGID(ID, GID);
        final SettingsSliderElementA sit = new SettingsSliderElementA(ID, GID, title, addr, value, _min, _max, type, _li);
        this.AddItem(ID, GID, sit, title, 0);
    }
    
    public void AddSettingsSliderElementB(String ID, String GID, String title, int addr, int value, int _min, int _max, String type, int _ind, XListSliderElementActionListener _li) {
        _checkIDGID(ID, GID);
        final SettingsSliderElementB sit = new SettingsSliderElementB(ID, GID, title, addr, value, _min, _max, type, _ind,  _li);
        this.AddItem(ID, GID, sit, title, 0);
    }
    
    /* **************************************************************************************************************************************
     * 
     * Этот блок нужен для того, чтобы в коде не было конструкций типа вот этого:
     *      что-то-там = (SettingsHexNumberBoxElement) xList1.get(GID).get(ID).GetComponent();
     * поскольку выглядит ужасно и так же плохо читаемо потом.
     * 
     *************************************************************************************************************************************** */

    public SettingsHexNumberBoxElement GetSettingsHexNumberBoxElement(String GID, String ID) {
        return (SettingsHexNumberBoxElement) ListElements.get(GID).get(ID).GetComponent();
    }
    
    public SettingsI2CAddressBoxElement GetSettingsI2CAddressBoxElement(String GID, String ID) {
        return (SettingsI2CAddressBoxElement) ListElements.get(GID).get(ID).GetComponent();
    }
    
    public SettingsComboBoxElement GetSettingsComboBoxElement(String GID, String ID) {
        return (SettingsComboBoxElement) ListElements.get(GID).get(ID).GetComponent();
    }
    
    public SettingsNumberBoxElement GetSettingsElementNumber(String GID, String ID) {
        return (SettingsNumberBoxElement) ListElements.get(GID).get(ID).GetComponent();
    }
    
    public SettingsTextBoxElement GetSettingsTextBoxElement(String GID, String ID) {
        return (SettingsTextBoxElement) ListElements.get(GID).get(ID).GetComponent();
    }
    
    public SettingsSliderElementA GetSettingsSliderElementA(String GID, String ID) {
        return (SettingsSliderElementA) ListElements.get(GID).get(ID).GetComponent();
    }
    
    public SettingsSliderElementB GetSettingsSliderElementB(String GID, String ID) {
        return (SettingsSliderElementB) ListElements.get(GID).get(ID).GetComponent();
    }
    ...
    public void ClearGroup(String GID) {
        if (ListElements.get(GID) == null) return;
        ListElements.get(GID).clear();
        ListTags.get(GID).clear();
    }
