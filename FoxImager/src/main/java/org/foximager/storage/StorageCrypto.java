...
import org.foximager.storage.Storage.CryptType;
...
public class StorageCrypto {
    private static final org.slf4j.Logger 
                logger = LoggerFactory.getLogger(StorageCrypto.class);
    ...
    public StorageCrypto(CryptType ct) {
        cryptType = ct;
    }
    
    public void setKeyRequestHandler(CryptActionListener c) {
        actionListener = c;
    }
    
    @SuppressWarnings("ConvertToTryWithResources")
    public void init(File storageDir) throws Exception {
        publicKeyFile  = new File(storageDir.getAbsolutePath() + File.separator + PUBLIC_KEY_NAME);
        privateKeyFile = new File(storageDir.getAbsolutePath() + File.separator + "private.key");
        
        isAES256Enable = (cryptType == CryptType.AES256) ? isAES256Support() : false;
        ...
        keystoreFile = new File(storageDir.getAbsolutePath() + File.separator + KEYSTORE_NAME);
        final CryptInfo ci;
    ...
    public byte[] crypt(byte[] value) {
        ...
        try {
            ...
            Logger.getLogger(StorageCrypto.class.getName()).log(Level.SEVERE, null, ex);
            return null;
        } 
    }
    ...
    public boolean isAES256Support() {
        final SecureRandom sr = new SecureRandom();
...
//            MessageBox.show("AES256 не поддерживается установленной в системе версией java! Созданная база будет не переносима на другие системы.\n "
//                    + "Пожалуйста, установите патч поддержки AES256 с официального сайта Oracle: http://www.oracle.com/technetwork/java/javase/downloads/jce8-download-2133166.html"
//                    + " и скажите спасибо за эту бесполезную трату времени устаревшим законам США относительно криптографии.");
